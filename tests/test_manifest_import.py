...
These run without git — member imports are served by an in-memory importer stub, self-imports by
real files under a temporary directory — except the few that exercise the git-tree route directly.
"""
...
def test_import_empty_map_reads_default_file():
    # Every import-map key is optional, so "import: {}" equals "import: true".
    manifest = load(
        """
        manifest:
          members:
...
def test_unknown_non_string_import_keys_reported():
    # A bare "on:" key is the boolean True after YAML resolution and does not sort against the
    # string keys.
    with pytest.raises(MalformedManifest) as excinfo:
        load(
            """
...
def test_empty_path_pattern_rejected(key, pattern):
    # A pattern with no components matches nothing and would crash PurePath.match; like other empty
    # strings it is rejected.
    with pytest.raises(MalformedManifest) as excinfo:
        load(
            "manifest:\n"
            "  members:\n"
...
def test_empty_member_import_rejected():
    # "" is not "no import": it would read the member's root directory, not fall back to the default
    # file. Like other empty strings, it is an editing artifact and rejected.
    with pytest.raises(MalformedManifest) as excinfo:
        load(
            """
            manifest:
...
def test_member_import_path_escape_rejected(block, fragment):
    # git refuses to read such a path, and the failure would otherwise be reported as a stale
...
def test_member_import_path_is_normalized_before_reading():
    # The importer (and git, on the direct route) is asked for the normalized path, as the
    # self-import route and the cycle check already use: git does not normalize an inner ".." in a
    # <rev>:<path> spec, and the failure would be reported as a stale repospace-rev, advising an
...
def test_duplicate_name_rejected_even_when_filtered():
    # A file declaring the same name twice is malformed regardless of which occurrences an importing
    # manifest's filter keeps.
    with pytest.raises(MalformedManifest) as excinfo:
        load(
            """
            manifest:
...
def test_path_prefix():
    ...
    # path-prefix scopes only to imported content; the importing member's own placement comes from
    # its "path" attribute.
...
def test_imported_path_of_prefix_directory_rejected():
    # "path: ." names the prefix directory, not a member of its own — the same mistake as "path: ."
    # at the top level.
    with pytest.raises(MalformedManifest) as excinfo:
        load(
            TOP_WITH_PREFIX,
...
def test_member_self_import_uses_member_data():
    # A "self: import:" inside a manifest imported from a member is resolved from the member's own
    # data (here: the importer), never from a filesystem working tree.
    manifest = load(
        TOP_WITH_CHILD,
        {
...
def test_self_import_symlinked_file_in_directory_rejected(tmp_path):
    # Manifest data is never read through a symlink, wherever the link points: the git route cannot
    # follow one, so following it here would make the same repository resolve differently by route.
    (tmp_path / "evil.yaml").write_text("manifest:\n")
    repo = tmp_path / "repo"
...
def test_self_import_broken_symlink_in_directory_rejected(tmp_path):
    # A link with a missing target is not a file, so it would be skipped silently while the git
    # route reports it as a symlink.
    (tmp_path / "sub").mkdir()
...
def test_self_import_symlink_inside_repository_rejected(tmp_path, target):
    # Not an escape: the link stays inside the manifest repository and was accepted before, file or
    # directory alike.
    (tmp_path / "sub").mkdir()
...
def test_self_import_through_symlinked_directory_rejected(tmp_path, imp):
    # Only the imported file used to be checked; a linked directory on the way to it was followed,
    # while the git route cannot resolve such a path at all. Every component below the repository
    # root counts, so both routes agree about the same repository.
    (tmp_path / "real" / "conf").mkdir(parents=True)
    (tmp_path / "real" / "inner.yaml").write_text(
        "manifest:\n  members:\n    - name: x\n      url: u\n"
    )
...
def test_self_import_default_file_cycle(tmp_path):
    # A self-import map without "file" defaults to repospace.yaml; at the top level that is the
    # document itself.
    top = tmp_path / "repospace.yaml"
    top.write_text("manifest:\n  self:\n    import:\n      path-prefix: p\n")
    with pytest.raises(MalformedManifest) as excinfo:
...
def test_import_depth_guard():
    ...
    # The failure is attributed to the member whose data was being resolved, not to "self".
...
def test_import_values_are_validated_when_imports_are_ignored(text, fragment):
    # "manifest --validate" does not follow imports; an import value of the wrong shape must still
    # fail there, not only on the next real load.
    with pytest.raises(MalformedManifest) as excinfo:
...
def test_unreadable_self_import_rejected(tmp_path):
    # A permission problem inside import resolution must surface as MalformedManifest, like the
    # top-level manifest read, not as a traceback from a raw OSError.
    top = tmp_path / "repospace.yaml"
    top.write_text("manifest:\n  self:\n    import: extra.yaml\n")
...
def test_importer_returning_none_skips():
    manifest = Manifest.from_data(
        textwrap.dedent(TOP_WITH_CHILD), importer=lambda member, path: None
    )
    assert names(manifest) == ["manifest", "child"]
...
def test_recursive_alias_in_self_import_rejected(tmp_path):
    # A YAML alias can make the import sequence contain itself; that must be a clean error, not a
...
def test_sibling_alias_in_import_stays_legal(tmp_path):
    # A diamond — the same anchored list aliased twice as siblings — is not a cycle.
    top = tmp_path / "repospace.yaml"
...
def test_member_import_of_symlink_rejected(repos, path):
    # git stores a symlink as a blob with mode 120000, so the object type alone would let the link
    # target text be parsed as manifest data — as a single-file import or inside a directory import.
    repo = repos.create(
        "child",
...
def test_member_import_of_regular_file_still_read(repos):
    ...
    repos.branch(repo, MANIFEST_REV)
    member = Member("child", url="unused", topdir=str(repos.base))
    # The symlink is not manifest data by name, so the directory still imports cleanly.
